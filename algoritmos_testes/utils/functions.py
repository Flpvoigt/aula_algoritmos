...
def custo_oportunidade(valor1, valor2):
    if valor1 > valor2:
        custo_oportunidade1 = valor1 - valor2
        return f"o custo beneficio entre o {valor1} e {valor2}, é de R$: {custo_oportunidade1}"
    elif valor2 > valor1:
        custo_oportunidade2 = valor2 - valor1
        return f"o custo beneficio entre o {valor2} e {valor1}, é de R$: {custo_oportunidade2}"
    else:
        return f"os dois valores possue o mesmo valor"

def calculos_bimestrais(valor1, valor2):
    resultado = (valor1 * 4 + valor2 * 6) / 10
    if resultado >= 7:
        return "Aluno aprovado"
    else:
        return "Aluno Reprovado" 

def saldo_conta_bancaria(saldo, salario, retirada):
    saldo = float(saldo + salario)

    if retirada <= saldo:
        saldo = saldo - retirada
        return f"vocÊ retirou R${retirada:.2F} e possui disponivel para saque ainda R${saldo:.2F}"
    else:
        return f"saldo baixo para esta retirada"
